<template>
    <div class="side-nav" :class="layout">
        <el-menu router ref="navbar" :default-active="defActive" :mode="navMode" menu-trigger="click" @select="selectMenu" @open="openMenu" @close="closeMenu" unique-opened>
            <nav-bar-item v-for="(item, n) in navList" :item="item" :navIndex="String(n)" :key="n" v-if="item.isShow"></nav-bar-item>
        </el-menu>
...
export default {
    ...
    watch: {
        ...
        $route(){
            ...
            if(this.$refs.navbar.items[path]!=undefined){
    ...
    created(){
      console.log(this.navList,'真实的菜单列表============')
    },
    methods: {
