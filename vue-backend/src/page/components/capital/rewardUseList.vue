	...
	<div class="sys-page">
		...
		<!-- 搜索 -->
						...
						<el-form-item prop="money">
						 ...
						 <el-form-item >
							...
							<el-form-item prop="reward_name">
							 ...
							 <el-form-item >
									 <el-button type="primary" @click="handleDownload()" v-hasPermission="'capital.rewardUseList.export'">导出</el-button>
							 </el-form-item>
					 </el-form>
			 </app-search>
				 <!-- 表格体 -->
			 <table-mixin pagination paginationAlign="center" :paginationTotal="searchForm.total" @getSize="getPageSize" @getCurrentPage="getCurrentPage">
					 ...
					 <div class="sum"> <div class="left" style="float:left">汇总：<span>人数：{{pNum}}</span> <span class="money">金额：{{money}}</span>
					 </div>	 <div style="float:right"><span>投资总额：{{invest_count}}</span><span  class="money">业绩总额：{{achive_count}}</span></div> </div>
			 </table-mixin>
	</div>
</template>
...
<script>
	...
	import { rewardUseListData} from '@/util/comData'
	export default{
		name:'const',
		data(){
			return rewardUseListData()
		},
		computed:{
			...mapState({
       ...
       getTableData() {
					 this.$axios({
					 		url: '/admin/capital/rewardUseList',
					 		method: 'post',
					 		...
					 		if(res.errcode==0){
									 ...
									 this.invest_count = res.data.sum.countTmoney
									 this.achive_count = res.data.sum.CountPerformance
					 		}
					 })
       },
			 //获取当前年月日
